defmodule Equivalex do
  ...
  defp differing_elements(a,b,_c) when is_map(a) and is_map(b) do
    differing_elements(Map.to_list(a) |> Enum.sort, Map.to_list(b) |> Enum.sort, 0)
  end
  ...
  defp differing_elements([a|ra],[b|rb],c), do: differing_elements(ra, rb, c ||| comparison(a,b))
  defp differing_elements(<<>>, <<>>, c), do: c
  ...
  defp comparison(a,b) when (is_list(a)   and is_list(b))
                       or   (is_tuple(a)  and is_tuple(b))
                       or   (is_map(a)    and is_map(b))
                       or   (is_binary(a) and is_binary(b)), do: differing_elements(a,b,0)
